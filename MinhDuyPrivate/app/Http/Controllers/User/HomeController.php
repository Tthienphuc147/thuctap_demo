...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        //     ->where('is_delete','=','0')
        //     ->get();
        $toan_bo_san_pham_dien_thoai = DB::table('san_phams')
            ->join('loai_san_phams','san_phams.id_loai_sp','=','loai_san_phams.id')
            ->join('danh_muc_san_phams','danh_muc_san_phams.id','=','loai_san_phams.id_danh_muc_sp')
            ->select('san_phams.*','danh_muc_san_phams.id as id_danh_muc_sp','danh_muc_san_phams.ten as ten_danh_muc_sp')
            ->where('san_phams.is_delete','=','0')
            ->where('danh_muc_san_phams.id','=','6')
            ->inRandomOrder()
            ->take($cai_dat_san_pham->so_luong_theo_danh_muc)
            ->get();
        $toan_bo_san_pham_lap_top = DB::table('san_phams')
            ->join('loai_san_phams','san_phams.id_loai_sp','=','loai_san_phams.id')
            ->join('danh_muc_san_phams','danh_muc_san_phams.id','=','loai_san_phams.id_danh_muc_sp')
            ->select('san_phams.*','danh_muc_san_phams.id as id_danh_muc_sp','danh_muc_san_phams.ten as ten_danh_muc_sp')
            ->where('san_phams.is_delete','=','0')
            ->where('danh_muc_san_phams.id','=','10')
            ->inRandomOrder()
            ->take($cai_dat_san_pham->so_luong_theo_danh_muc)
            ->get();
        $toan_bo_san_pham_tablet = DB::table('san_phams')
            ->join('loai_san_phams','san_phams.id_loai_sp','=','loai_san_phams.id')
            ->join('danh_muc_san_phams','danh_muc_san_phams.id','=','loai_san_phams.id_danh_muc_sp')
            ->select('san_phams.*','danh_muc_san_phams.id as id_danh_muc_sp','danh_muc_san_phams.ten as ten_danh_muc_sp')
            ->where('san_phams.is_delete','=','0')
            ->where('danh_muc_san_phams.id','=','7')
            ->inRandomOrder()
            ->take($cai_dat_san_pham->so_luong_theo_danh_muc)
            ->get();
        ...
        // ));
        return view('user.pages.home')->with(compact('cai_dat_san_pham','san_pham_theo_danh_muc','toan_bo_san_pham_dien_thoai','toan_bo_san_pham_lap_top','toan_bo_san_pham_tablet','toan_bo_san_pham_moi','toan_bo_san_pham_khuyen_mai','toan_bo_san_ban_chay','ba_loai_sp_ngau_nhien'));
